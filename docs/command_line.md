...
-  **epsilon** : convergence criterion for the fast sweeping method (FSM, see Qian et al. 2007). The sweeps stop once the **mean per-node change** in traveltime between two successive iterations falls below this fraction of the **range** of the traveltime field (max − min over the nodes reached). The criterion is therefore dimensionless: the same value behaves the same whatever grid size, physical units or source origin time the model uses. Default is 1.e-6, which is where the error against an analytic solution was measured to plateau at every resolution tested; 1.e-5 left the WENO3 pass short of that plateau on grids finer than about 200². Note: the WENO3 update is non-monotone early on — the change per sweep rises before it falls, from a first value that shrinks as O(h²) — so a pass is never allowed to stop while its change is still at or above the previous sweep's. Without that guard a fine enough grid would stop after a single WENO sweep.
-  **max number of iteration** : max number of sweeping iterations (FSM) default is 200. This is a ceiling, not a fixed count, so raising it costs nothing on models that converge earlier; 50 was too low for the WENO3 pass on grids finer than about 200², where it capped the sweeps before **epsilon** was met. A sweep loop that runs out of iterations before reaching **epsilon** has not converged, and the solver then writes a warning naming the pass (first-order or WENO3) and the mean per-node change it stopped at. From Python, `get_niter()` and `get_niterw()` report the counts of the last `raytrace` call.
