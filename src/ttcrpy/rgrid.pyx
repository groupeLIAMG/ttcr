...
cdef class Grid3d_f:
    ...
    Same as Grid3d_d but uses float32 for coordinates, slowness, traveltimes,
    and raypaths.  Use the Grid3d factory to select precision via dtype=.
    ...
    def __cinit__(self, np.ndarray[np.float32_t, ndim=1] x,
        ...
        if cell_slowness:
            ...
            if method == 'FSM':
                ...
                self.method = b's'
                self.grid = new Grid3Drnsp[float,uint32_t](nx, ny, nz,
                                                           self._dx, self._dy, self._dz,
                                                           xmin, ymin, zmin,
                                                           nsnx, nsny, nsnz,
                                                           tt_from_rp, interp_vel,
                ...
                self.method = b'd'
                self.grid = new Grid3Drndsp[float,uint32_t](nx, ny, nz,
                                                            self._dx, self._dy, self._dz,
                                                            xmin, ymin, zmin,
                                                            n_secondary, tt_from_rp,
                                                            n_tertiary, radius_factor_tertiary,
    ...
    def x(self):
        """np.ndarray: node coordinates along x"""
        tmp = np.empty((self._x.size(),), dtype=np.float32)
        cdef int n
        for n in range(self._x.size()):
            tmp[n] = self._x[n]
        return tmp
    ...
    def y(self):
        """np.ndarray: node coordinates along y"""
        tmp = np.empty((self._y.size(),), dtype=np.float32)
        cdef int n
        for n in range(self._y.size()):
            tmp[n] = self._y[n]
        return tmp
    ...
    def z(self):
        """np.ndarray: node coordinates along z"""
        tmp = np.empty((self._z.size(),), dtype=np.float32)
        cdef int n
        for n in range(self._z.size()):
            tmp[n] = self._z[n]
        return tmp
    ...
    def get_grid_traveltimes(self, thread_no=0):
        ...
        cdef int n
        self.grid.getTT(tmp, thread_no)
        tt = np.empty((tmp.size(),), dtype=np.float32)
        for n in range(tmp.size()):
            tt[n] = tmp[n]
        shape = (self._x.size(), self._y.size(), self._z.size())
        return tt.reshape(shape, order='F')
    ...
    def get_s0(self, hypo, slowness=None):
        ...
        cdef vector[vector[sxyz[float]]] vTx
        vTx.resize(nTx)
        ...
        for n in range(nTx):
            for nn in range(evID.size):
                if eid[n] == evID[nn]:
                    i0 = nn
                    break
            vTx[n].push_back(sxyz[float](src[i0,0], src[i0,1], src[i0,2]))

        for i in eid:
            ii = evID == i
            iTx.append(np.nonzero(ii)[0])

        s0 = np.zeros((src.shape[0],), dtype=np.float32)
    ...
    def raytrace(self, source, rcv, slowness=None, thread_no=None,
        ...
        if source.shape[1] == 5:
            ...
            src = source
            _, ind = np.unique(source, axis=0, return_index=True)
            Tx = source[np.sort(ind), :]
            t0 = np.zeros((Tx.shape[0],))
            nTx = Tx.shape[0]
        elif source.shape[1] == 4:
            src = source[:,1:4]
            _, ind = np.unique(source, axis=0, return_index=True)
            tmp = source[np.sort(ind), :]
            nTx = tmp.shape[0]
            Tx = tmp[:,1:4]
            t0 = tmp[:,0]
        else:
        ...
        cdef vector[vector[sxyz[float]]] vTx
        cdef vector[vector[sxyz[float]]] vRx
        cdef vector[vector[float]] vt0
        cdef vector[vector[float]] vtt
        ...
        if evID is None:
            if nTx == 1:
                vTx[0].push_back(sxyz[float](src[0,0], src[0,1], src[0,2]))
                for r in rcv:
                    vRx[0].push_back(sxyz[float](r[0], r[1], r[2]))
                vt0[0].push_back(t0[0])
                vtt[0].resize(rcv.shape[0])
                iRx.append(np.arange(rcv.shape[0]))
            elif aggregate_src:
                for t in Tx:
                    vTx[0].push_back(sxyz[float](t[0], t[1], t[2]))
                for t in t0:
                    vt0[0].push_back(t)
                for r in rcv:
                    vRx[0].push_back(sxyz[float](r[0], r[1], r[2]))
                ...
                for n in range(nTx):
                    ind = np.sum(Tx[n,:] == src, axis=1) == 3
                    iRx.append(np.nonzero(ind)[0])
                    vTx[n].push_back(sxyz[float](Tx[n,0], Tx[n,1], Tx[n,2]))
                    vt0[n].push_back(t0[n])
                    for r in rcv[ind,:]:
                        vRx[n].push_back(sxyz[float](r[0], r[1], r[2]))
                    vtt[n].resize(vRx[n].size())
        else:
            if src.shape != rcv.shape:
                raise ValueError('src and rcv should be of equal size')
        ...
        if self._n_threads == 1:
            ...
            if return_rays:
                ...
                rays = []
                for n2 in range(vRx.size()):
                    r = np.empty((r_data[0][n2].size(), 3), dtype=np.float32)
                    for nn in range(r_data[0][n2].size()):
                        r[nn, 0] = r_data[0][n2][nn].x
                        r[nn, 1] = r_data[0][n2][nn].y
                        r[nn, 2] = r_data[0][n2][nn].z
        ...
        if return_rays:
            ...
            for n in range(nTx):
                r = [ [0.0] for i in range(vRx[n].size())]
                for n2 in range(vRx[n].size()):
                    r[n2] = np.empty((r_data[n][n2].size(), 3), dtype=np.float32)
                    for nn in range(r_data[n][n2].size()):
                        r[n2][nn, 0] = r_data[n][n2][nn].x
                        r[n2][nn, 1] = r_data[n][n2][nn].y
                        r[n2][nn, 2] = r_data[n][n2][nn].z
    ...
    def builder(filename, size_t n_threads=1, str method='FSM',
        ...
        data = reader.GetOutput()
        x = numpy_support.vtk_to_numpy(data.GetXCoordinates()).astype(np.float32)
        y = numpy_support.vtk_to_numpy(data.GetYCoordinates()).astype(np.float32)
        z = numpy_support.vtk_to_numpy(data.GetZCoordinates()).astype(np.float32)
        ...
        if 'lowness' in name:
            slowness = data.reshape(dim, order='F').flatten().astype(np.float32)
        else:
            slowness = (1.0 / data.reshape(dim, order='F').flatten()).astype(np.float32)
...
cdef class Grid2d_f:
    ...
    Same as Grid2d_d but uses float32 for coordinates, slowness, traveltimes,
    and raypaths.  Use the Grid2d factory to select precision via dtype=.
    ...
    cdef Grid2D[float,uint32_t,sxz[float]]* grid
    ...
    def __cinit__(self, np.ndarray[np.float32_t, ndim=1] x,
        ...
        if cell_slowness:
            if method == 'SPM':
                ...
                self.method = b'd'
                self.grid = new Grid2Drcdsp[float,uint32_t,sxz[float]](
                                    nx, nz, self._dx, self._dz,
                                    xmin, zmin, n_secondary, n_tertiary,
                                    radius_factor_tertiary, tt_from_rp,
                                    use_edge_length, n_threads)
            ...
            if method == 'SPM':
                ...
                self.method = b'd'
                self.grid = new Grid2Drndsp[float,uint32_t,sxz[float]](nx, nz,
                                self._dx, self._dz, xmin, zmin,
                                n_secondary, n_tertiary,
                                radius_factor_tertiary, tt_from_rp,
                                use_edge_length, n_threads)
    ...
    def get_slowness(self):
        ...
        if self.cell_slowness:
            ...
            nz = nz - 1
        slown_size = nx * nz
        slowness = np.ndarray((slown_size,), dtype=np.float32)
        for i in range(slown_size):
            slowness[i] = slown[i]
        return slowness.reshape((nx, nz))
    ...
    def get_s0(self, hypo, slowness=None):
        ...
        cdef vector[vector[sxz[float]]] vTx
        vTx.resize(nTx)
        ...
        for n in range(nTx):
            for nn in range(evID.size):
                if eid[n] == evID[nn]:
                    i0 = nn
                    break
            vTx[n].push_back(sxz[float](src[i0,0], src[i0,1]))
    ...
    def raytrace(self, source, rcv, slowness=None, xi=None, theta=None,
        ...
        cdef vector[vector[sxz[float]]] vTx
        cdef vector[vector[sxz[float]]] vRx
        cdef vector[vector[float]] vt0
        cdef vector[vector[float]] vtt
        ...
        iRx = []
        if nTx == 1:
            vTx[0].push_back(sxz[float](src[0,0], src[0,1]))
            for r in rcv:
                vRx[0].push_back(sxz[float](r[0], r[1]))
            vt0[0].push_back(t0[0])
            vtt[0].resize(rcv.shape[0])
            iRx.append(np.arange(rcv.shape[0]))
        elif aggregate_src:
            for t in Tx:
                vTx[0].push_back(sxz[float](t[0], t[1]))
            for t in t0:
                vt0[0].push_back(t)
            for r in rcv:
                vRx[0].push_back(sxz[float](r[0], r[1]))
            ...
            for n in range(nTx):
                ind = np.sum(Tx[n,:] == src, axis=1) == 2
                iRx.append(np.nonzero(ind)[0])
                vTx[n].push_back(sxz[float](Tx[n,0], Tx[n,1]))
                vt0[n].push_back(t0[n])
                for r in rcv[ind,:]:
                    vRx[n].push_back(sxz[float](r[0], r[1]))
                vtt[n].resize(vRx[n].size())
        ...
        if self._n_threads == 1:
            ...
            if return_rays:
                ...
                rays = []
                for n2 in range(vRx.size()):
                    r = np.empty((r_data[0][n2].size(), 2), dtype=np.float32)
                    for nn in range(r_data[0][n2].size()):
                        r[nn, 0] = r_data[0][n2][nn].x
                        r[nn, 1] = r_data[0][n2][nn].z
                    rays.append(r)
        ...
        if return_rays:
            ...
            for n in range(nTx):
                r = [ [0.0] for i in range(vRx[n].size())]
                for n2 in range(vRx[n].size()):
                    r[n2] = np.empty((r_data[n][n2].size(), 2), dtype=np.float32)
                    for nn in range(r_data[n][n2].size()):
                        r[n2][nn, 0] = r_data[n][n2][nn].x
                        r[n2][nn, 1] = r_data[n][n2][nn].z
                for nt in range(vtt[n].size()):
...
def Grid3d(x, y, z, n_threads=1, cell_slowness=1, method='FSM',
    ...
    ----------
    x, y, z : array-like
        Node coordinates.  Converted to the requested dtype automatically.
    dtype : numpy dtype
        np.float64 (default) for double precision (Grid3d_d),
        np.float32 for single precision (Grid3d_f).
...
Grid3d.builder = Grid3d_d.builder
...
def Grid2d(x, z, n_threads=1, cell_slowness=1, method='SPM', aniso='iso',
    ...
    Factory that returns a 2D rectilinear-grid raytracer with the requested
    numeric precision.
    ...
    """
    if np.dtype(dtype) == np.dtype(np.float64):
        return Grid2d_d(np.asarray(x, dtype=np.float64),
                        np.asarray(z, dtype=np.float64),
...
Grid2d.data_kernel_straight_rays = Grid2d_d.data_kernel_straight_rays
