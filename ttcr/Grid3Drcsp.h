...
namespace ttcr {
    ...
    public:
        ...
        /// Grid3D::raytrace(), which traces the raypath by following the
    ...
    private:
         ...
         * is wanted, so they share this one body.
         *
    ...
    template<typename T1, typename T2, typename CELL>
    template<typename SIV>
    void Grid3Drcsp<T1,T2,CELL>::raytraceSensitivity(const std::vector<sxyz<T1>>& _Tx,
                                                     const std::vector<T1>& t0,
                                                     const std::vector<sxyz<T1>>& _Rx,
                                                     std::vector<T1>& traveltimes,
                                                     std::vector<std::vector<sxyz<T1>>>* r_data,
        ...
        for (size_t n=0; n<Rx.size(); ++n) {
            ...
            // check if Rx is on one of the Tx nodes: the walk below would
            // otherwise start from a node that has no parent
            bool foundTx = false;
            for ( size_t nt=0; nt<Tx.size(); ++nt ) {
                if ( Tx[nt] == Rx[n] ) {
                    traveltimes[n] = t0[nt];
