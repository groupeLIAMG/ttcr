...
namespace ttcr {
                                                  ...
                                                  const size_t nt)
    {
        ...
        size_t nNodes = static_cast<size_t>(nnodes[0]) * nnodes[2];
        std::vector<T> slowness( static_cast<size_t>(ncells[0])*ncells[2] );
        ...
        switch (par.method) {
            ...
            case DYNAMIC_SHORTEST_PATH:
            {
                ...
                if ( par.time ) { begin = std::chrono::high_resolution_clock::now(); }
                if ( constCells )
                    g = new Grid2Drcdsp<T, uint32_t, sxz<T>>(ncells[0], ncells[2],
                                                             d[0], d[2],
                                                             min[0], min[2],
                                                             par.nn[0],
                                                             par.nTertiary,
                                                          ...
                                                          const size_t nt)
    {
        ...
        switch (par.method) {
            ...
            case FAST_MARCHING:
            {
                ...
                if ( par.time ) { begin = std::chrono::high_resolution_clock::now(); }
                if ( constCells )
                    g = new Grid2Ducfm<T, uint32_t, sxz<T>>(nodes,
                                                            triangles,
                                                            par.tt_from_rp,
                                                            nt);
                else
                    g = new Grid2Dunfm<T, uint32_t, sxz<T>>(nodes,
                                                            triangles,
                                                            par.tt_from_rp,
                                                            nt);
                if ( par.time ) { end = std::chrono::high_resolution_clock::now(); }
            ...
            case FAST_SWEEPING:
            {
                ...
                if ( par.time ) { begin = std::chrono::high_resolution_clock::now(); }
                if ( constCells )
                    g = new Grid2Ducfs<T, uint32_t, sxz<T>>(nodes,
                                                            triangles,
                                                            par.epsilon,
                                                            par.nitermax,
                                                            par.tt_from_rp,
                    ...
                    g = new Grid2Dunfs<T, uint32_t, sxz<T>>(nodes,
                                                            triangles,
                                                            par.epsilon,
                                                            par.nitermax,
                                                            par.tt_from_rp,
                ...
                if ( constCells )
                    dynamic_cast<Grid2Ducfs<T, uint32_t, sxz<T>>*>(g)->initOrdering( ptsRef, par.order );
                else
                    dynamic_cast<Grid2Dunfs<T, uint32_t, sxz<T>>*>(g)->initOrdering( ptsRef, par.order );
                if ( par.time ) { end = std::chrono::high_resolution_clock::now(); }
                if ( verbose ) {
                    std::cout << "done.\n";
                    std::cout << "Initiated ordering with " << ptsRef.size() << " reference points and l-"
                                                              ...
                                                              const size_t nt)
    {
        ...
        if ( par.time ) { begin = std::chrono::high_resolution_clock::now(); }
        if ( constCells )
            g = new Grid2Ducsp<T, uint32_t, sxyz<T>, Node3Dcsp<T,uint32_t>, Cell<T, Node3Dcsp<T,uint32_t>, sxyz<T>>>(nodes,
                                                                                                                     triangles,
                                                                                                                     par.nn[0],
                                                                                                                     par.tt_from_rp,
                                                                                                                     nt);
        else
            g = new Grid2Dunsp<T, uint32_t, sxyz<T>, Node3Dnsp<T,uint32_t>>(nodes,
                                                                            triangles,
                                                                            par.nn[0],
                                                                            par.tt_from_rp,
                                                                            nt);
