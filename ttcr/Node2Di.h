...
//    Node2Di(const T1 xx, const T1 zz, const T2 index, const int nt=1) :
//	nThreads(nt),
//    tt(0),
//	x(xx), z(zz), slowness(0.0), 
//    gridIndex(index),
//    nodeParent(0),
//    cellParent(0),
//    owners(0)
...
//    Node2Di(const T1 t, const T1 xx, const T1 zz, const int nt, const size_t i) :
//	nThreads(nt),
//    tt(0),
//	x(xx), z(zz), slowness(0.0), 
//    gridIndex(std::numeric_limits<T2>::max()),
...
//	}
    ...
    T1 getSlowness() const { return slowness; }
    void setSlowness(const T1 s) { slowness = s; }
