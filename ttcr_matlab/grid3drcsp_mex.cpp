...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
            ...
            for ( size_t i=0; i<num_threads-1; ++i ) {
                ...
                threads[i]=thread( [&grid_instance,&vTx,&tt,&t0,&Rx,&iTx,&nRx,
                    ...
                    for ( size_t nv=blk_start; nv<blk_end; ++nv ) {
                        ...
                        if ( nlhs == 3 ) {
                            try {
                                grid_instance->raytrace(vTx[nv], t0[nv], vRx, tt[nv], r_data[nv], l_data[nv], i+1);
                            } catch (...) {
                                mexErrMsgTxt("Problem while raytracing.");
                            }
                        } else if ( nlhs == 2 ) {
    ...
    if (!strcmp("get_nsx", cmd)) {
        ...
        if ( nrhs > 2 ) {
            mexErrMsgTxt("get_nsx: No arguments needed.");
        }
        if (nlhs > 1) {
            mexErrMsgTxt("get_nsx: has a maximum of one output argument.");
        }
    ...
    if (!strcmp("get_nsy", cmd)) {
        // Check parameters
        
        if ( nrhs > 2 ) {
            mexErrMsgTxt("get_nsy: No arguments needed.");
        }
        if (nlhs > 1) {
            mexErrMsgTxt("get_nsy: has a maximum of one output argument.");
        }
        
        uint32_t nt = grid_instance->getNsny();
        /* ------------------------------------------------------
         Output variable
         ------------------------------------------------------ */
    ...
    if (!strcmp("get_nsz", cmd)) {
        ...
        if ( nrhs > 2 ) {
            mexErrMsgTxt("get_nsz: No arguments needed.");
        }
        if (nlhs > 1) {
            mexErrMsgTxt("get_nsz: has a maximum of one output argument.");
        }
